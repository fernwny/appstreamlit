...
list the first verse to ninth and the vocabulary of the song in a JSON array
...
-tenth line should be the vocabulary in the song with the type in the bracket and meaning 
"""
...
if st.button('Let\'s go'):
    ...
    if isinstance(sd, list) :
        ...
        for i, verse in enumerate(sd.values(), 1):  # Exclude the last item (vocab)
            if i == 5:
                st.write(f"Chorus: {verse}")
            elif i >= 6:
                st.write(f"Verse {i-1}: {verse}")
